...
`r funlink(data.table)` package instead.
:::
...
`r funlink(sf)`, which stands for *simple features*, is the main R
...
`r funlink(terra)` provides a general framework for working with spatial
