...
open-graph:
  title: |
    IPUMS DHS Research Hub — Using Leaflet for Exploratory Data Analysis: Part 1
  description: |
...
You can find more detailed information about the specific methodologies 
used to produce these data sources on the GHSL website.
...
posts](../2024-08-01-ndvi-data/#nasa-image-files). The GHSL interface provides you with an
interactive map where you can point and click on the tiles you want to
download.

::: column-page
![](images/ghsl_download.png)
:::
...
you hover over individual tiles.

::: column-page
![](images/ghsl_download2.png)
:::

We'll download data for 2010 and 2020. You can do so by selecting these
years in the *Epoch* selection grid on the left side of the screen and
clicking on the same tiles again.

::: column-page
![](images/ghsl_download3.png)
:::
...
::: column-margin
Each layer needs to be assigned levels individually. You could iterate
to do this if you wanted to apply levels to a larger raster with more
layers, but here we simply assign levels manually for each layer
:::
...
the `addProviderTiles()` function.

We're going to use the Dark Matter basemap tiles from
...
rewrite this code. We'll also use `setView()` to set the default map
view to be centered on Ethiopia, since this will be our area of
interest:
...
type of geometry layer we add (this is similar to the different `geom_`
...
layer (we'll just plot the 2010 data for now):
...
Mercator. Even though our GHS data is in a
[Mollweide](https://desktop.arcgis.com/en/arcmap/latest/map/projections/mollweide.htm)
projection, leaflet displays the raster in Web Mercator. This is why the
raster boundaries appear distorted from what we may have expected based
on the GHSL website.

If we turn off this default behavior, we'll notice the discrepancy in
our data and basemap (which is in Web Mercator). See how the coastlines
in the Red Sea appear offset between the raster and the basemap if we
*don't* project our data:
...
Fortunately, because our `ghs_smod` raster is **categorical**,
`project()` will automatically use a raster resampling method that
preserves this property.

However, if the input raster were still in **numeric** format,
resampling would likely introduce intermediate values that would no
longer belong to one of the density categories provided by the GHS!
:::
