...
area. We summarized several of the main population distribution data sources
in our [previous post](../2024-08-30-pop-density/), so take a look there for an
overview.
...
produce the GHSL in our [previous post](../2024-08-30-pop-density/) and on
the GHSL website.
...
you hover over individual tiles on the world map (see the red oval in the image
above).

We'll download data for 2010 and 2020. You can do so by selecting these
years in the *Epoch* selection grid on the left side of the screen (see the
red square in the image above) and clicking on the same tiles again.
...
::: column-margin
Each layer (i.e. epoch) needs to be assigned levels individually. For raster
stacks containing more epochs, you may want to do this iteratively. Since we
are only using two epochs, we'll simply assign levels manually for each layer.
:::
...
The `setView()` function allows us to specify the centerpoint and initial
...
rewrite this code. We'll also update our view parameters to set the default map
...
our data and basemap (which is also in Web Mercator). See how the coastlines
...
represented as a **categorical** raster,
...
resampling would likely produce some intermediate cell values that would no
