...
-   RStudio
-   Git
-   A University of Minnesota github.com account
...
which requires RStudio. You can [download RStudio here](https://rstudio.com/products/rstudio/download/).
...
on the private site, which is hosted in the `umn-isrdi` organization on
GitHub at <https://github.com/umn-isrdi/dhs-research-hub>.

Access to the private GitHub repository is limited to approved
collaborators. By working in this environment, we can develop and edit
new posts without having to worry about them becoming visible on the
public site prematurely. Also, it allows us to store files necessary for
certain posts (e.g., IPUMS data files) without making those files
publicly visible. Members of the blog "admin" team will be responsible
for migrating completed posts to the public version of the site.

Before continuing, make sure you can log in to
<https://github.com> with the GitHub account that has been granted
...
```{r}
#| fig-alt: "RStudio and Git"
#| fig-align: center
knitr::include_graphics("images/bpw/git-menu.png")
```

### Configure Git with your GitHub credentials {#configure-git}

Finally, we'll provide Git with the username and email associated with
the GitHub account you'll use for the private repository. We'll also
need to generate a Personal Access Token (PAT) for our account. The PAT
functions like a password that allows you to interact with GitHub from R
and the command line.

First, set the username and email address associated with your GitHub
account. Git commands need to be run in the RStudio Terminal.
...
```{r, eval = F, echo = T}
usethis::create_github_token(host = "https://github.com")
```
...
```{r}
#| fig-alt: "RStudio and Git Step 3"
knitr::include_graphics("images/bpw/use_git.png")
```

On the next menu page, enter the address for the internal GitHub
repository:
`https://github.com/umn-isrdi/dhs-research-hub.git`
...
```{r}
#| fig-alt: "RStudio and Git Step 4"
knitr::include_graphics("images/bpw/clone_info.png")
```

::: callout-warning
Make sure to clone the **private** version of the repository (the one in
the `umn-isrdi` organization), not the **public** version (located in the
`ipums` organization).
:::
...
::: column-page
![](images/bpw/clone_success.png){fig-alt="RStudio and Git Step 5"}
:::
...
::: column-page
![](images/bpw/post_added.png){fig-alt="Post added"}
:::
...
-   `image-alt`: the alt text to be associated with the thumbnail image specified by the `image` attribute. `image-alt` 
    is required unless `image` hasn't been set, in which case, the default first post image and its associated alt text will be used instead.
...
image-alt: "VIIRS image of NDVI data"
open-graph:
  title: "From MODIS to VIIRS: The Latest Source for NDVI Data"
  description: "Update your NDVI workflows with NASA's newest instruments"
  image: index_files/figure-html/listing-img-1.png
...
internal GitHub repository, but will prevent those data from being published
publicly.

::: callout-tip
If you're working with any data that you don't even want to publish to
the **internal GitHub repository** (e.g., if the data files are too large), put
...
To ensure that content on the blog is usable by the widest possible 
audience, please follow these accessibility best practices when drafting your post:

1. Provide meaningful alt text for all images
All images should include descriptive alternative text (`fig-alt` in Quarto). This allows screen reader users to 
understand the content and purpose of the image.

2. Use headings in a logical order
Headings help structure your post and allow readers (and assistive technologies) to navigate it efficiently. In
particular, don't skip heading levels (e.g., go from `##` to `###`, not directly to `####`)

3. Run an accessibility check with WAVE
Before committing or publishing your post, please run a quick accessibility scan using the WAVE tool.

Since you're likely viewing your post on your local machine, your best bet is installing the WAVE browser 
extension (available for all major browsers) and running it directly on your previewed post.

Review any errors or alerts flagged by WAVE and address them where possible. Common issues include missing alt text, 
skipped headings, and unlabeled elements.

### Push your post to GitHub

When you're finished writing, you'll need to share (or *push*) your
changes to the internal GitHub repository.

::: callout-note
Pushing to the internal GitHub repository is not the same thing as
publishing your post to the live website. Your post won't go live until
someone from the admin team merges your post into the main branch and
migrates it to the public GitHub repository.
:::
...
Now, if you visit the internal GitHub repository, your post will appear in
a new branch!

::: column-page
![](images/bpw/see_branches.png){fig-alt="Post in new branch"}
:::
...
get the latest copy of their branch from the internal GitHub repository. If
...
```{r}
#| fig-alt: "Toggle between branches"
...
Now, if you visit our internal GitHub repository, you'll see that your edited
...
Before adding a second remote, it's best to rename the internal GitHub remote
...
repository](https://github.com/umn-isrdi/dhs-research-hub), click [Pull
Requests ‣ New Pull Request]{.menu-instr}.

::: column-page
![](images/bpw/new_pr.png){fig-alt="Pull request"}
:::
...
::: column-page
![](images/bpw/site_from_internal.png){fig-alt="Site from internal"}
:::
...
::: column-page
![](images/bpw/squash_merge.png){fig-alt="Squash and merge"}
:::
